...
async function inspect(
...
): Promise<CatalogSnapshotMigrationState> {
  const [identityResult, migrationResult, schemaResult, catalogResult] = await Promise.all([
    db.execute(sql`
    ...
    `),
    db.execute(sql`
      SELECT to_regclass('public.ax_catalog_health_snapshots') IS NOT NULL AS has_table
    `),
    db.execute(sql`SELECT count(*)::int AS c FROM catalog_items`),
  ])
  ...
  return {
    ...
    latestMigrationTimestamp: typeof migrations.latest_created_at === 'string'
      ...
      : null,
    hasSnapshotTable: schema.has_table === true,
    catalogItemCount: count((catalogResult.rows[0] as Record<string, unknown>)?.c),
...
function summary(label: string, state: CatalogSnapshotMigrationState): void {
  console.log(
    `${label}: branch=${state.actualBranchId ?? 'unknown'}, migrations=${state.migrationCount}, ` +
      `snapshotTable=${state.hasSnapshotTable ? 'present' : 'absent'}, catalogItems=${state.catalogItemCount}`
  )
}
