/** Guarded AX 0038 cron_runs table migration runner for Neon. */
...
const PRODUCTION_CONFIRMATION = 'apply-ax-0038'
...
async function inspect(
...
): Promise<CronRunsMigrationState> {
  const [identityResult, migrationResult, schemaResult, catalogResult] = await Promise.all([
    ...
    `),
    db.execute(sql`
      SELECT to_regclass('public.cron_runs') IS NOT NULL AS has_table,
             EXISTS (SELECT 1 FROM pg_type WHERE typname = 'cron_run_status') AS has_enum
    `),
  ...
  return {
    ...
    latestMigrationTimestamp: typeof migrations.latest_created_at === 'string'
      ...
      : null,
    hasCronRunsTable: schema.has_table === true,
    hasStatusEnum: schema.has_enum === true,
    skillEventCount: count((catalogResult.rows[0] as Record<string, unknown>)?.c),
...
function summary(label: string, state: CronRunsMigrationState): void {
  console.log(
    `${label}: branch=${state.actualBranchId ?? 'unknown'}, migrations=${state.migrationCount}, ` +
      `cronRuns=${state.hasCronRunsTable ? 'present' : 'absent'}, enum=${state.hasStatusEnum ? 'present' : 'absent'}, ` +
      `skillEvents=${state.skillEventCount}`
  )
}
...
async function main(): Promise<void> {
  ...
  summary('AX 0038 preflight', before)
  assertSafe('AX 0038 preflight', validateCronRunsBeforeMigration(before, production))
  if (!apply) {
    console.log(production
      ? `Ready. Re-run with --apply --confirm-production-migration ${PRODUCTION_CONFIRMATION}.`
      : 'Ready. Re-run the same command with --apply to execute migration 0038.')
    return
  }
  ...
  summary('AX 0038 verification', after)
  assertSafe('AX 0038 verification', validateCronRunsAfterMigration(after, production))
  console.log('AX 0038 cron run log table applied and verified.')
}
