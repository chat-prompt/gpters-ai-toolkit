...
/**
 * 이 패널의 하루 경계는 전부 KST다 (`./kst` 참고).
 * 이벤트 수를 세는 스킬 사용량 패널은 UTC 경계를 유지한다(문서화된 선택).
 */
/**
...
export const overviewPanel: AxPanel<AxOverviewData> = {
  ...
  async load({ days, isAdmin }) {
    ...
    try {
      // 왕복 지연이 쌓이지 않게 서로 독립인 쿼리를 한 번에 보낸다.
      // 365일 잔디는 기간과 무관하므로 activity-grass 패널이 따로 조회한다.
      const [[cumulative], [catalog], hourlyRows, flowResult, memberRows] = await Promise.all([
        // 1. 누적 참여 인원 — 전 기간
        db
