...
작성 2026-09-21. 이 문서 하나로 두 작업을 이어받을 수 있게 썼다. **여기부터 읽는다.**

두 작업은 성격이 달라 이슈를 나눴다. 각 이슈 본문만으로도 그 트랙을 끝까지 할 수 있다.

| 트랙 | 이슈 | 브랜치 | worktree |
|---|---|---|---|
| A. 재랭킹 (정확도) | [DEV-4484](https://linear.app/geniefy/issue/DEV-4484) | `hayoungchoi/dev-4484` | `.claude/worktrees/dev-4484-search-rerank` |
| B. 지연 (성능) | [DEV-4485](https://linear.app/geniefy/issue/DEV-4485) | `hayoungchoi/dev-4485` | `.claude/worktrees/dev-4485-search-latency` |

둘 다 `origin/main`(819533fb)에서 땄고 준비만 해뒀다. 아직 아무 변경도 없다.

**둘 다 할 거면 B를 먼저 본다.** A가 지연을 0.6~0.9초 더하는데, 지금 이미 목표의 5배다.
...
**`packages/lib/src/search/vector-search.ts:137~232`** — `results`가 배열로 materialize된 직후, `:232`의 `return` 직전.
...
Vercel 서버리스는 인스턴스가 자주 죽는다. 먼저 **실제 히트율을 재고**, 프로세스 밖(DB 테이블 또는 외부 KV)으로 옮긴다. 키 정규화 규칙은 지금 것을 유지한다.
