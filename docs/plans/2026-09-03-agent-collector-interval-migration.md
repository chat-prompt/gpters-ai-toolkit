...
2026-09-03 기준 운영 DB는 아직 0034 미적용이다.

- 기록된 마이그레이션 23건, 마지막은 `0033_ax_skill_journeys`(`1788141600000`)
- `interval_seconds` 컬럼 기본값 `21600`
- 등록된 수집기 2개(`bbodoong`, `bbokeoter`) 모두 `interval_seconds = 3600`

기존 수집기가 이미 1시간이므로 **적용 전에도 운영 동작은 정상이다.** 이 마이그레이션은 앞으로의
새 설치가 기본값을 그대로 쓰게 만드는 것이 목적이다.
