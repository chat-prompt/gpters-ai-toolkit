...
| `errorCooldownMs` | 5분 | JEV가 오류를 돌려주면(시간 초과 제외) 이 시간 동안 JEV를 부르지 않고 기존 방식으로 검색한다 |
...
- **JEV가 오류를 돌려주면 그 뒤 5분은 키가 없을 때와 똑같이 동작한다.** 오류에는 HTTP 오류(401·429·529 등), 응답 누락, 네트워크 실패가 들어간다. 오류가 반복되는 동안 검색마다 JEV 왕복(약 0.4초)을 치르지 않기 위해서다. **시간 초과는 오류로 치지 않는다.** 느린 호출은 그 요청만 원래 순서로 넘기고, 다음 검색에서는 다시 JEV를 부른다. 휴지 상태는 함수 인스턴스마다 따로 둔다.
...
- 운영 함수(sin1)에서 잰 지연은 아래 Preview 절에 있다.

## Preview(sin1) 측정 (2026-09-21 05:52 UTC)

PR #163의 Preview 배포(`dpl_6Ei6iSM2MXGbYWRjS5DUNWiojcnF`, 제한 시간 800ms, 키 등록됨)에 같은 질의 15개를 `limit=5`로 두 번씩 보냈다. 읽기 호출만 했다.

- **싱가포르에서 부른 JEV는 415~539ms(p50 약 440ms)다.** 한국에서 부를 때(220~280ms)보다 느리다.
- JEV를 부르지 않은 검색(임베딩 확신): 약 300~330ms. 2회차에는 임베딩 캐시가 맞아 약 30ms였다.
- JEV를 부른 검색: 1회차 743~875ms, 2회차(임베딩 캐시 히트) 453~511ms.
- **인스턴스의 첫 호출은 JEV가 805ms에서 시간 초과로 끊겼다.** 원래 순서로 폴백했고, 그래서 그 한 번은 슬랙 요약 질의가 카카오톡 도구를 1등으로 돌려줬다. 이 호출은 임베딩도 콜드였다(2,148ms).
- 정답: 1회차 13/14(첫 호출의 시간 초과 폴백), **2회차 14/14**.
- JEV를 부른 9개 중 5개는 `jev_uncertain`(JEV도 가르지 못함)이었다. 이 경우 약 0.4초를 쓰고도 순서는 그대로다. 배포 후 실데이터로 `embeddingGapBelow`를 낮출지 판단한다.
