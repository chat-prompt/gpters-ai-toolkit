...
import { isIncidentReviewer, resolveAccountUserId, resolveAxViewer } from '@/lib/features/ax'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(await enrollStoredReportInbox(parsed.data, new URL(request.url).origin, reviewerId), { status: 201, headers })
  } catch (error) {
