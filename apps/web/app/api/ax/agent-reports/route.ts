...
import { allowedReportWorkspace, incidentReportSchema, submitIncidentReport } from '@/lib/features/ax'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!parsed.success) return reportError('보고 항목과 명시적 승인 메시지 링크를 확인하세요',400)
    if (!allowedReportWorkspace([parsed.data.issueUrl,parsed.data.approvalUrl])) return reportError('허용된 Slack 워크스페이스의 링크가 필요합니다',400)
