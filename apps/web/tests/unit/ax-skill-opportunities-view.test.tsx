  ...
  groups: [
    {
      ...
      total: 12,
      skills: [
        { skillId: 'quiet', name: '조용한 스킬', shown: 194, loaded: 5, applied: 0, skipped: 5, appliers: 0, anonymousApplies: 0 },
      ],
    },
    ...
    { category: 'no_outcome', total: 0, skills: [] },
    {
      ...
      total: 1,
      skills: [
        { skillId: 'solo', name: '혼자 쓰는 스킬', shown: 12, loaded: 9, applied: 5, skipped: 0, appliers: 1, anonymousApplies: 0 },
      ],
    },
  ],
  searchRequests: 1219,
  observedSearches: 1219,
...
describe('AX 스킬 개선 기회 화면', () => {
  it('후보가 있는 분류만 기준·근거 수치와 함께 보여준다', () => {
    ...
    expect(screen.queryByText('로드는 많고 적용은 적음')).toBeNull()
    expect(screen.queryByText('결과 보고가 없음')).toBeNull()

    expect(screen.getByText('노출은 많고 로드는 적음')).toBeTruthy()
    // 총합 비교임을 밝히고 전환이라 말하지 않는다
    expect(screen.getByText(/총합/)).toBeTruthy()
    // 전체 수와 보여준 수를 함께 밝힌다
    ...
    expect(screen.getByText('기준 · 적용 3건 이상, 보고한 계정 1개, 계정 불명 보고 없음')).toBeTruthy()

    // 스킬 이름은 행 머리칸(th scope=row)이라 스크린리더가 각 수치의 주어로 읽는다
    // 노출·로드·적용·건너뜀이 한 줄에 그대로 실린다
    const quietCells = [...screen.getByText('조용한 스킬').closest('tr')!.querySelectorAll('th, td')]
      .map((cell) => cell.textContent)
    expect(quietCells).toEqual(['조용한 스킬', '194', '5', '0', '5'])

    // 한 사람만 쓰는 분류는 마지막 칸이 건너뜀 대신 사람 수다
    const soloCells = [...screen.getByText('혼자 쓰는 스킬').closest('tr')!.querySelectorAll('th, td')]
      .map((cell) => cell.textContent)
    expect(soloCells).toEqual(['혼자 쓰는 스킬', '12', '9', '5', '1개'])

    // 결과 0건 검색은 전체 대비로 적는다
    const zeroResult = screen.getByText(/결과를 확인할 수 있는 검색 1,219건/)
