...
describe('MCP Handlers', () => {
  ...
  describe('executeTool', () => {
    ...
    // --- EDU-6275: rate_skill (human feedback) ---

    it('should execute rate_skill with rating and comment', async () => {
      const result = await executeTool('rate_skill', {
        skillId: 'code-reviewer',
        rating: 5,
        comment: 'PR 리뷰 시간이 절반으로 줄었습니다',
      })

      expect(result.isError).toBeUndefined()
      const parsed = JSON.parse(result.content[0].text)
      expect(parsed.success).toBe(true)
      expect(parsed.message).toBe('Rating recorded')
      expect(parsed.skillId).toBe('code-reviewer')
      expect(parsed.rating).toBe(5)
      expect(result._meta).toBeDefined()
      expect(result._meta?.skillRating).toEqual({
        skillId: 'code-reviewer',
        rating: 5,
        comment: 'PR 리뷰 시간이 절반으로 줄었습니다',
      })
    ...
    it('should execute rate_skill without comment', async () => {
      const result = await executeTool('rate_skill', {
        skillId: 'refactor-guide',
        rating: 3,
      })
    ...
    it('should return error for rate_skill without skillId', async () => {
      const result = await executeTool('rate_skill', {
        rating: 5,
      })

      expect(result.isError).toBe(true)
      expect(result.content[0].text).toContain('Missing required fields: skillId, rating')
    })

    it('should return error for rate_skill without rating', async () => {
      const result = await executeTool('rate_skill', {
        skillId: 'test-skill',
      })
    ...
    it('should return error for rate_skill with invalid rating (0)', async () => {
      const result = await executeTool('rate_skill', {
        skillId: 'test-skill',
        rating: 0,
      })
      ...
      expect(result.isError).toBe(true)
      expect(result.content[0].text).toContain('rating must be an integer between 1 and 5')
    })

    it('should return error for rate_skill with invalid rating (6)', async () => {
      const result = await executeTool('rate_skill', {
        skillId: 'test-skill',
        rating: 6,
      })
    ...
    it('should return error for rate_skill with non-integer rating', async () => {
      const result = await executeTool('rate_skill', {
        skillId: 'test-skill',
        rating: 3.5,
      })
