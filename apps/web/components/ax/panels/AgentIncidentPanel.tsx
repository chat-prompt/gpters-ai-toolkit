...
function ReportEvidence({record}:{record:IncidentCase}) {
  ...
  return <div className="space-y-4 text-sm">
    ...
    <div className="flex flex-wrap gap-3"><EvidenceLink value={report.issueUrl} label="문제 대화 열기 ↗"/>{report.approvalUrl && <EvidenceLink value={report.approvalUrl} label="보고 요청·승인 메시지 ↗"/>}</div>
    {report.reaction && <div className="space-y-1 rounded-lg border border-[var(--border-hover)] p-3 text-xs"><p>이모지 보고 요청 · :{report.reaction.name}: · {formatDateTime(new Date(Number(report.reaction.eventTs)*1000).toISOString())}</p><p>에이전트가 전달한 반응 정보입니다. 접수 요청이며 해결 승인이 아닙니다.</p><p className="break-all">이벤트 {report.reaction.eventId} · 요청자 {report.reaction.userId}</p></div>}
    <p className="text-xs text-[var(--text-secondary)]">보고자 {report.reporterAgentId} · 요청자 {report.requestedBy} (에이전트가 전달한 정보)<br/>{report.reaction ? '반응과 원문 접근 권한은 별도로 확인합니다.' : '슬랙 승인 원문은 검토자가 링크에서 확인합니다.'} 작업 ID {report.taskId ?? '미연결'}</p>
