...
function BootstrapLine({summary}:{summary:ObservationSummary}){
  ...
  const text=!value?capabilityText[capability]:!value.sessions?'이 기간에 부팅 스냅샷 없음'
    :`${headroom!==null?`여유 ${number(headroom)}자 (한도 ${number(value.fileCharsLimit!)}자의 ${number(share!)}% 사용)`:`가장 큰 파일 ${number(value.largestFileCharsLatest!)}자 (한도 모름)`} · 잘림 ${value.truncatedSessions}세션 · 경고 ${value.warningSessions}세션 · 스냅샷 ${value.sessions}개 · 기간 최대 ${number(value.largestFileCharsMax!)}자`
  ...
  const tight=Boolean(value&&(value.truncatedSessions>0||(share!==null&&share>=90)))
  return <>
