...
export async function GET(request: NextRequest) {
  ...
  // For localhost/127.0.0.1, allow flexible port matching for CLI tools
  const isValidRedirectUri = client.redirectUris.some((registeredUri) => {
    // Exact match first
    if (registeredUri === redirectUri) return true;

    // For localhost URIs, allow different ports (CLI tools use random ports)
    try {
      ...
      if (isLocalhost(registered.hostname) && isLocalhost(requested.hostname)) {
        // Match protocol and pathname, allow different ports
        return (
      ...
      // URL parsing failed, fall through to exact match
    }
