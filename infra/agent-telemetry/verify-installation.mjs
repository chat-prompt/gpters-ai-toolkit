...
function execute(node, cli, args) {
  ...
  if (args[0] === '--version') {
    // Node may emit warnings before or after the CLI's stderr version line.
    const versions = `${result.stdout}\n${result.stderr}`.split(/\r?\n/).filter(line => /^aitk v\S+$/.test(line))
    if (versions.length !== 1) throw new Error('CLI version output is missing or ambiguous')
    return versions[0]
  }
